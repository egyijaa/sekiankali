        ...
        <div class="row page-titles mx-0">
            ...
            <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active"><a href="javascript:void(0)">UoMs</a></li>
                </ol>
            </div>
        </div>
        <!-- row -->
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header justify-content-between d-flex d-inline">
                        <h4 class="card-title">Daftar UoM</h4>
                        @if($errors->any())
                        ...
                        @endif
                        @if (auth()->user()->can('add mous'))
                        <button type="button" data-target="#modalTambah" data-toggle="modal" class="btn"><i class="btn btn-sm btn-primary shadow-sm">+ UoM Baru</i></button>
                        @endif
                      </div>
                    <div class="card-body">
                                    ...
                                    <tr class="selected">
                                        ...
                                        <td>
                                            <div class="btn-group mb-2 btn-group-sm">
                                                @if (auth()->user()->can('edit mous'))
                                                <button class="btn btn-info cek" type="button" data-target="#modalUpdate"
                                                data-toggle="modal" data-id="{{ $item->id }}" data-mou="{{ $item->mou }}"><i class="fa fa-pencil"
                                                        aria-hidden="true"></i></button>
                                                @endif
                                                @if (auth()->user()->can('delete mous'))
                            ...
                            <div class="card-body">
                                <div class="basic-form">
                                    <form class="form-valide-with-icon-update"
                                        action="{{ route('mous.tambah') }}" method="POST"
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body">
                                <div class="basic-form">
                                    <form class="form-valide-with-icon-update"
                                        action="{{ route('mous.edit') }}" method="POST"
                                        enctype="multipart/form-data">
                                        @csrf
                                        @method('PUT')
...
<div class="modal fade" id="delete" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form action="{{ route('mous.hapus') }}" method="POST">
                @csrf
                @method('delete')
                <input type="hidden" name="id">
                <div class="modal-header">
...
<script>
    ...
    $('#modalUpdate').on('show.bs.modal', (e) => {
        ...
        $('#modalUpdate').find('input[name="mou"]').val($(e.relatedTarget).data('mou'));
